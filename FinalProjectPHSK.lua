...
-- Update  : - Hapus Button Reset Manual (Full Otomatis)
--           - Auto Reset Use: 0 & Dup: 0 setiap selesai ronde/keluar meja
--           - List Kata refresh otomatis untuk game baru
--           - Single Github DB (ZellRayyKBBI) & Blacklist Permanen
-- =================================================================
...
local function RegisterWord(w)
    ...
    -- [STRICT FILTER] Kurang dari 3 huruf -> BLACKLIST
    if #wl < 3 then
        BlacklistDB[wl] = true
...
local function UpdateInfoUI()
    if LblInfo then
        ...
        local p  = isMobile and "[HP]" or "[PC]"
        local aj = isAutoJoin and " | AJ:ON" or ""
        LblInfo.Text = p.." "..m..aj.." | Auto-Save DB Aktif"
    end
end
...
local THEME = {
    ...
    AJOn           = Color3.fromRGB(0,190,100),
    AJOff          = Color3.fromRGB(200,50,50),
}
...
Frame.AnchorPoint            = isMobile and Vector2.new(0.5, 0) or Vector2.new(0.5, 0.5)
Frame.Position               = isMobile and UDim2.new(0.5, 0, 0, 8) or UDim2.new(0.5, 0, 0.5, 0)
...
Title.Text = "PrawiraHub - Sambung Kata"; Title.TextColor3 = THEME.TitleColor
...
makeVDiv(232); makeVDiv(573)

-- ── KOLOM KIRI (INFO) ───────────────────────────────────────────────
local LX = 15; local LW = 210; local LY = 52; local LH = 17

local function makeLbl(y, txt, col, fs)
    local l = Instance.new("TextLabel", Frame)
    l.Size = UDim2.new(0,LW,0,LH); l.Position = UDim2.new(0,LX,0,y)
...
LblJoin    = makeLbl(LY,      "● Belum join meja",        THEME.Red,    11)
LblGiliran = makeLbl(LY+19,   "Giliran: -",                Color3.fromRGB(200,150,255), 11)
LblNama    = makeLbl(LY+37,   "Nama: -",                   THEME.Yellow, 11)
LblSpy     = makeLbl(LY+55,   "Ngetik: -",                 THEME.Cyan,   11)
makeSep(LY+76)
LblPre     = makeLbl(LY+81,   "HURUF AWAL: -",             THEME.Yellow, 12)
LblTyping  = makeLbl(LY+100,  "TARGET: -",                 THEME.Cyan,   11)
LblStatus  = makeLbl(LY+118,  "Status: Booting...",        THEME.Yellow, 11)
LblDBStat  = makeLbl(LY+137,  "DB:0 Dup:0 Use:0 Blk:0",    Color3.fromRGB(150,255,150), 10)
makeSep(LY+157, THEME.TitleColor)
LblRiwayatValue = makeLbl(LY+162, "Riwayat: -",            THEME.Pink,   11)
makeSep(LY+183, THEME.StrokeColor)

LblAutoJoinTarget = makeLbl(LY+188, "Target: "..selectedTable,  THEME.Yellow, 11)
LblAutoJoinStatus = makeLbl(LY+206, "AJ Status: IDLE",          THEME.Cyan,   11)

local function makeRiwBtn(y, txt, bg)
    local b = Instance.new("TextButton", Frame)
    b.Size = UDim2.new(0,LW,0,25); b.Position = UDim2.new(0,LX,0,y)
...
local BtnRiwayatDB        = makeRiwBtn(LY+238, "📚 Riwayat New Local DB",  THEME.BtnExport)
local BtnRiwayatBlacklist = makeRiwBtn(LY+268, "🚫 Riwayat Blacklist",     THEME.BtnDelete)

-- ── KOLOM TENGAH (Sliders, Dropdowns, Import, Output) ────────────────
local MX = 241; local MW = 325; local MY = 52
local function makeMSep(y, col)
    local s = Instance.new("Frame", Frame)
...
local function makeSlider(posY, labelText, initVal, minVal, maxVal, fillColor, onChanged)
    ...
    con.Size = UDim2.new(0,MW,0,22); con.Position = UDim2.new(0,MX,0,posY); con.BackgroundTransparency = 1
    local lbl = Instance.new("TextLabel", con)
    lbl.Size = UDim2.new(0,90,1,0); lbl.BackgroundTransparency = 1; lbl.Text = labelText
    ...
    line.Size = UDim2.new(1,-90,0,5); line.Position = UDim2.new(0,90,0.5,-2)
    ...
    hit.Size = UDim2.new(1,-90,1,0); hit.Position = UDim2.new(0,90,0,0)
...
makeSlider(MY,    "Delay: 0.7s",  currentTurnDelay, MIN_TURN_DELAY, MAX_TURN_DELAY, THEME.Neon,
    function(v,l) currentTurnDelay=v; l.Text="Delay: "..string.format("%.1f",v).."s" end)
makeSlider(MY+28, "Ketik: 0.05s", TypingSpeed, MIN_SPEED, MAX_SPEED, THEME.TitleColor,
    function(v,l) TypingSpeed=v; l.Text="Ketik: "..string.format("%.2f",v).."s" end)
makeSlider(MY+56, "Hapus: 0.05s", DeleteSpeed, MIN_DEL_SPEED, MAX_DEL_SPEED, THEME.Red,
...
CreateDropdown(200, MX, MY+92, filterModes, currentFilterIndex, function(idx)
    currentFilterIndex=idx; UpdateInfoUI()
    if TriggerListRefresh then TriggerListRefresh() end
end)
CreateDropdown(115, MX+210, MY+92, sortModes, currentSortIndex, function(idx)
    currentSortIndex=idx; UpdateInfoUI()
    if TriggerListRefresh then TriggerListRefresh() end
end)

local ImportBox = Instance.new("TextBox", Frame)
ImportBox.Size = UDim2.new(0,MW,0,22); ImportBox.Position = UDim2.new(0,MX,0,MY+122)
...
BtnRow.Size = UDim2.new(0,MW,0,24); BtnRow.Position = UDim2.new(0,MX,0,MY+150)
BtnRow.BackgroundTransparency = 1
local BtnRowLayout = Instance.new("UIListLayout", BtnRow)
BtnRowLayout.FillDirection = Enum.FillDirection.Horizontal; BtnRowLayout.SortOrder = Enum.SortOrder.LayoutOrder; BtnRowLayout.Padding = UDim.new(0,4)
...
local BtnExp  = makeSmBtn(61, "Exp DB",   THEME.BtnExport, 1)
local BtnImp  = makeSmBtn(61, "Imp DB",   THEME.BtnImport, 2)
local BtnExpB = makeSmBtn(61, "Exp Blk",  THEME.BtnExport, 3)
local BtnImpB = makeSmBtn(61, "Imp Blk",  THEME.BtnImport, 4)
local BtnDel  = makeSmBtn(61, "Clr All",  THEME.BtnDelete, 5)

LblInfo = Instance.new("TextLabel", Frame)
LblInfo.Size = UDim2.new(0,MW,0,14); LblInfo.Position = UDim2.new(0,MX,0,MY+180)
...
BtnOutputToggle.Size = UDim2.new(0, 155, 0, 24); BtnOutputToggle.Position = UDim2.new(0, MX, 0, MY+200)
...
BtnCopyOutput.Size = UDim2.new(0, 165, 0, 24); BtnCopyOutput.Position = UDim2.new(0, MX+160, 0, MY+200)
...
OutScroll.Size = UDim2.new(0,MW,0,195); OutScroll.Position = UDim2.new(0,MX,0,MY+230)
...
-- ── KOLOM KANAN (Auto Play, Auto Join, Search, Word List) ────────────────
local RX = 579; local RW = 356; local RY = 52
local function makeRCSep(y, col)
    local s = Instance.new("Frame", Frame)
...
BtnPlay.Size = UDim2.new(0,RW,0,33); BtnPlay.Position = UDim2.new(0,RX,0,RY)
BtnPlay.BackgroundColor3 = THEME.BtnStop; BtnPlay.Text = "Auto Play Off"
BtnPlay.TextColor3 = THEME.TextColor; BtnPlay.Font = THEME.Font; BtnPlay.TextSize = 12
AddStyle(BtnPlay, 6)
...
makeRCSep(RY+40, THEME.TitleColor)

local AJTitleLbl = Instance.new("TextLabel", Frame)
AJTitleLbl.Size = UDim2.new(0,RW,0,14); AJTitleLbl.Position = UDim2.new(0,RX,0,RY+44)
AJTitleLbl.BackgroundTransparency = 1; AJTitleLbl.Text = "─── AUTO JOIN TABLE ───"
...
AJScroll.Size = UDim2.new(0,RW,0,92); AJScroll.Position = UDim2.new(0,RX,0,RY+62)
...
BtnAutoJoin.Size = UDim2.new(0,RW,0,28); BtnAutoJoin.Position = UDim2.new(0,RX,0,RY+160)
...
SearchBox.Size = UDim2.new(0,RW,0,25); SearchBox.Position = UDim2.new(0,RX,0,RY+205)
...
Scroll.Size = UDim2.new(0,RW,0,203); Scroll.Position = UDim2.new(0,RX,0,RY+235)
...
local DelOverlay, DelScale = createOverlay("Are you sure delete ALL DB?", function()
    ...
    SaveDatabases()
    UpdateDBStatUI()
    AddLog("All Databases CLEARED")
end)

local CloseOverlay, CloseScale = createOverlay("Are you sure you want to close?", function()
    SaveDatabases() -- Menyimpan DB & Blacklist sesaat sebelum ditutup sepenuhnya
    scriptActive = false
    ...
    -- Membersihkan Memori Table PrawiraHub agar bersih total dari RAM
...
-- Reset ketika player bangun / keluar dari meja
...
-- ============================================================
-- SNIFFER (Menangkap Keberhasilan Musuh)
-- ============================================================
...
-- ============================================================
-- getPrefix (ROBUST DETECTION)
-- ============================================================
local function getPrefix()
    local prefix = nil
    pcall(function()
        if currentPlayerTurn and currentPlayerTurn.Character then
            local head = currentPlayerTurn.Character:FindFirstChild("Head")
            if head then
                local bb = head:FindFirstChild("TurnBillboard")
                if bb then
                    local txtLbl = bb:FindFirstChild("Text")
                    if txtLbl and txtLbl:IsA("TextLabel") then
                        local t = string.gsub(txtLbl.Text, "<[^>]+>", "")
                        t = string.gsub(t, "[%s%p]", "")
                        if t ~= "" and #t <= 3 and t:lower() ~= "label" and t:lower() ~= "textbox" then
                            prefix = t
                            return
                        end
                    end
        ...
        if not prefix then
            ...
            if mUI and mUI:FindFirstChild("BottomUI") and mUI.BottomUI.Visible then
                local ws = mUI.BottomUI:FindFirstChild("TopUI") 
                    and mUI.BottomUI.TopUI:FindFirstChild("WordServerFrame") 
                    and mUI.BottomUI.TopUI.WordServerFrame:FindFirstChild("WordServer")
...
local function TypeSingleWord(kata, passedPrefix)
    ...
    if isAccepted then
        ...
        if LblStatus then LblStatus.Text = "❌ DITOLAK SERVER"; LblStatus.TextColor3 = THEME.Red end
        ...
        -- Masukkan ke blacklist karena kata ditolak Game Server
...
local function ExecuteAutoType(prefix, searchPool)
    ...
    for _, kata in ipairs(searchPool) do
        ...
        if strikes >= 3 then LblStatus.Text="BAHAYA! 3x Gagal, Manual!"; LblStatus.TextColor3=THEME.Red; task.wait(2); break end
...
BtnImp.MouseButton1Click:Connect(function()
    ...
    if ok and type(data)=="table" then
        ...
        for k,v in pairs(data) do
            if not LocalDB[k] and #tostring(k) > 2 and not BlacklistDB[k] then
                LocalDB[k]=v; cnt=cnt+1
            else totalDuplicates=totalDuplicates+1 end
        end
        SaveDatabases(); UpdateDBStatUI()
        LblStatus.Text="Imported "..cnt.." kata!"; LblStatus.TextColor3=THEME.Neon; ImportBox.Text=""
    else LblStatus.Text="JSON Tidak Valid!"; LblStatus.TextColor3=THEME.Red end
end)
BtnImpB.MouseButton1Click:Connect(function()
    local jt = tostring(ImportBox.Text)
    if jt=="" then LblStatus.Text="Paste JSON Blacklist dulu!"; LblStatus.TextColor3=THEME.Red; return end
    local ok, data = pcall(function() return HttpService:JSONDecode(jt) end)
    if ok and type(data)=="table" then
        local cnt=0
        for k,_ in pairs(data) do
            if not BlacklistDB[k] then
                BlacklistDB[k]=true; if LocalDB[k] then LocalDB[k]=nil end; cnt=cnt+1
            end
        end
        SaveDatabases(); UpdateDBStatUI()
        LblStatus.Text="Imported "..cnt.." Blacklist!"; LblStatus.TextColor3=THEME.Neon; ImportBox.Text=""
    else LblStatus.Text="JSON Tidak Valid!"; LblStatus.TextColor3=THEME.Red end
end)
